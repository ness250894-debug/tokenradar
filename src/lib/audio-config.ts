 ...
 *
 * Maps local audio files in src/data/audio/ to their beat-drop sync points.
...
export interface AudioTrack {
  /** Filename in src/data/audio/ (without path prefix). */
  file: string;
 ...
 * Used for Remotion's staticFile() or direct path resolution.
 */
export function getAudioPath(track: AudioTrack): string {
  return `src/data/audio/${track.file}`;
}
