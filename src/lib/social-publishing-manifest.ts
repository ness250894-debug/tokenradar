...
export function renderSocialPublishingRunbook(manifest = getSocialPublishingManifest()): string {
  ...
  return [
    ...
    "",
    "The collector reads published post IDs from D1 and writes cumulative snapshots to `social_post_metrics`. Telegram reads the exact post's public channel-preview view count using `TELEGRAM_CHANNEL_USERNAME`; unavailable interaction fields remain null. X uses the existing bearer token. Instagram follows explicit `IG_AUTH_MODE`: `facebook_login` uses `graph.facebook.com` with a Page/System User token and the `instagram_basic`, `instagram_content_publish`, `pages_read_engagement`, and `instagram_manage_insights` permissions; `instagram_login` uses `graph.instagram.com` with an Instagram User token and `instagram_business_basic`, `instagram_business_content_publish`, and `instagram_business_manage_insights`. The weekly Meta maintenance job converts an eligible Facebook User token once to its linked non-expiring Page token or renews an Instagram Login token with `ig_refresh_token`. Threads requires `threads_basic`, `threads_content_publish`, and `threads_manage_insights`. YouTube uses an API key for public statistics and OAuth for Analytics retention data. Re-run `scripts/generate-youtube-token.ts` once if the refresh token predates the `youtube.force-ssl`, `youtube.readonly`, and `yt-analytics.readonly` scopes. TikTok publishing remains paused, but historical post metrics can be collected through Display API `video.list` credentials; missing credentials produce an explicit skip instead of fabricated values.",
    "",
