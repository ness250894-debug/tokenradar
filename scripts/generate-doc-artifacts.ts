...
function synchronizeAutomationRunbook(rawMarkdown: string): string {
  ...
  synchronized = replaceMarkdownSection(synchronized, "Credential and Token Rotation", [
    ...
    "- before consuming a single-use X token, `src/lib/x-client.ts` preflights the current value over stdin to the `X_OAUTH2_REFRESH_TOKEN` secret in the serialized `social-automation` GitHub Actions environment, then immediately persists the replacement before publishing",
    "- only X route steps receive the narrowly scoped `Environments: write` credential, and the `x-token-refresh` manual route rotates the credential without posting",
    "",
    ...
    "- Threads continues to use `th_refresh_token`; current and renewed tokens self-inspect through the Threads `debug_token` endpoint, must match `THREADS_ACCOUNT_ID`, and must carry the basic, publishing, and insights scopes",
